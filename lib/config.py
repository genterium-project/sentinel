...
sentinel_cfg = GentariumConfig.tokenize(sentinel_config_file)
sentinel_version = "1.2.0"
min_gentariumd_proto_version_with_sentinel_ping = 70207


def get_gentarium_conf():
    if sys.platform == 'win32':
        gentarium_conf = os.path.join(os.getenv('APPDATA'), "GentariumCore/gentarium.conf")
    else:
        home = os.environ.get('HOME')

        gentarium_conf = os.path.join(home, ".gentariumcore/gentarium.conf")
        if sys.platform == 'darwin':
            gentarium_conf = os.path.join(home, "Library/Application Support/GentariumCore/gentarium.conf")

    gentarium_conf = sentinel_cfg.get('gentarium_conf', gentarium_conf)

    return gentarium_conf
...
gentarium_conf = get_gentarium_conf()
network = get_network()
