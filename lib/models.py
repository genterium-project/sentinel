...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, gentariumd):
        golist = gentariumd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_gentariumd(gentariumd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_gentariumd(self, gentariumd, rec):
        import decimal
        import gentariumlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from gentariumd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from gentariumd, with every run
        ...
        # get/create, then sync payment amounts, etc. from gentariumd - Gentariumd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(gentariumd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from gentariumd! %s" % e)
            govobj.vote_delete(gentariumd)
            return (govobj, None)
    ...
    def vote_delete(self, gentariumd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(gentariumd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, gentariumd, signal, outcome):
        import gentariumlib
        ...
        output = gentariumd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = gentariumlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(gentariumd, signal)

    def sync_network_vote(self, gentariumd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = gentariumd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = GENTARIUMD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 gentarium addr, non-multisig
            if not gentariumlib.is_valid_gentarium_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Gentarium address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Gentarium Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (gentariumlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = GENTARIUMD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not gentariumlib.is_valid_gentarium_address(addr, config.network):
    ...
    def hash(self):
        import gentariumlib
        return gentariumlib.hashit(self.serialise())
