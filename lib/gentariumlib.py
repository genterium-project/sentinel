...
def is_valid_gentarium_address(address, network='mainnet'):
    ...
    gentarium_version = 140 if network == 'testnet' else 38
    ...
    try:
        ...
        # rescue from exception, not a valid Gentarium address
        return False

    if (address_version != gentarium_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.gentariumd_serialise())
...
def SHIM_serialise_for_gentariumd(sentinel_hex):
    ...
    # superblock => "trigger" in gentariumd
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # gentariumd expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    gentariumd_hex = serialise(obj)
    return gentariumd_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('gentarium.conf')
