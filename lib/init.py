...
def has_gentarium_conf():
    import config
    import io

    valid_gentarium_conf = False

    # ensure gentarium_conf exists & readable
    #
    # if not, print a message stating that Gentarium Core must be installed and
    # configured, including JSONRPC access in gentarium.conf
    try:
        f = io.open(config.gentarium_conf)
        valid_gentarium_conf = True
    except IOError as e:
        print(e)

    return valid_gentarium_conf
...
def main():
    ...
    if not has_gentarium_conf():
        print("GentariumCore must be installed and configured, including JSONRPC access in gentarium.conf")
        sys.exit(1)
