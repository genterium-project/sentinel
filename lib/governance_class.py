...
class GovernanceClass(object):
    ...
    def vote(self, gentariumd, signal, outcome):
        return self.go.vote(gentariumd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, gentariumd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(gentariumd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(gentariumd, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import gentariumlib
        obj_data = gentariumlib.SHIM_serialise_for_gentariumd(self.serialise())
    ...
    def gentariumd_serialise(self):
        import gentariumlib
        return gentariumlib.SHIM_serialise_for_gentariumd(self.serialise())
