...
class GentariumDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some gentariumd methods
    ...
    @classmethod
    def from_gentarium_conf(self, gentarium_dot_conf):
        from gentarium_config import GentariumConfig
        config_text = GentariumConfig.slurp_config_file(gentarium_dot_conf)
        creds = GentariumConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = gentariumlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = gentariumlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = gentariumlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_gentariumd_proto_version_with_sentinel_ping)
